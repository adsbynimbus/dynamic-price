...
import com.adsbynimbus.NimbusError.ErrorType.RENDERER_ERROR
import com.adsbynimbus.dynamicprice.DynamicPriceRewardedAd
...
@Deprecated("Use loadDynamicPriceRewardedAd instead", ReplaceWith(""))
@SuppressLint("RestrictedApi")
fun RewardedInterstitialAd.showAd(
    activity: Activity,
    nimbusAd: NimbusResponse,
    @Suppress("Unused") nimbusAdManager: NimbusAdManager,
    @Suppress("Deprecation") callback: NimbusRewardCallback,
) {
    fullScreenContentCallback = object : FullScreenContentCallback() {
        ...
        override fun onAdFailedToShowFullScreenContent(error: AdError) {
            callback.onError(NimbusError(RENDERER_ERROR, error.message, null))
        }
    ...
    if (adMetadata.getString("AdSystem").equals("Nimbus", ignoreCase = true)) {
        ...
        activity.loadBlockingAd(nimbusAd)
            ?.let { controller ->
                ...
                controller.start()
            } ?: callback.onError(NimbusError(RENDERER_ERROR, "No renderer for ad", null))
    } else {
        show(activity) {
...
fun RewardedAd.showAd(
    ...
    DynamicPriceRewardedAd(this, nimbusAd, null).show(activity) {
        callback.onUserEarnedReward(rewardItem)
    }
}

@Deprecated("Use loadDynamicPriceRewardedAd instead",
    replaceWith = ReplaceWith("", "com.adsbynimbus.dynamicprice"))
